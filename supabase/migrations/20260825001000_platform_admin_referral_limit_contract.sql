-- Forward-only extension of the existing Platform Admin Control Center.
-- The restaurant-scoped monthly referral limit was introduced by 06000 and is
-- exposed here without duplicating referral settings or inventing fallbacks.
...
comment on function public.get_platform_restaurant_control_center(uuid) is
  'Read-only, Platform Admin authorized restaurant control-center aggregation including the authoritative restaurant referral invite limit. Missing telemetry is returned as unavailable, never as an invented healthy state.';
