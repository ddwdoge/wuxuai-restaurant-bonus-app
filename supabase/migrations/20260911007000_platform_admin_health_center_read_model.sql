-- Phase 5: one read-only, data-minimal Platform Admin health snapshot.
-- The snapshot composes existing protected evidence sources in one statement.
-- It creates no persistence, performs no repair and changes no tenant policy.
begin;
...
begin
  ...
  operational_payload := public.get_platform_operational_telemetry();
  country_payload := public.get_platform_country_launch_status();
