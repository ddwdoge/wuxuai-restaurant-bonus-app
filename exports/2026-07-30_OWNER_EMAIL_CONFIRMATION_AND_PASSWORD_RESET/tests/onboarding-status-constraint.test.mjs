...
const completionMigration = await readFile(
  new URL(
    "../supabase/migrations/20260730002000_onboarding_initial_legal_package_publication.sql",
    import.meta.url,
  ),
  "utf8",
);
...
test("Aktivierung bleibt Update-only, slug-stabil und completed", () => {
  assert.match(completionMigration, /update public\.restaurants[\s\S]*status = 'active'/);
  assert.match(completionMigration, /onboarding_status = 'completed'/);
  assert.doesNotMatch(activation, /\b(insert|upsert)\s*\(/i);
  assert.doesNotMatch(activation, /\bslug\s*:/i);
  assert.doesNotMatch(completionMigration, /insert into public\.restaurants/i);
