...
test("Onboarding veröffentlicht das initiale Legal-Paket vor erfolgreicher Aktivierung", () => {
  const generation = completionMigration.indexOf("perform public.generate_restaurant_legal_package");
  const publication = completionMigration.indexOf("perform public.publish_restaurant_legal_drafts");
  const activation = completionMigration.indexOf("onboarding_status = 'completed'", publication);
  const result = onboardingService.indexOf("return { restaurant");
  assert.ok(generation > 0);
  assert.ok(publication > generation);
  assert.ok(activation > publication);
  assert.match(onboardingService, /rpc\("complete_restaurant_onboarding"/);
  assert.ok(result > 0);
});
...
test("zukünftiges Gültigkeitsdatum wird vor der Aktivierung blockiert", () => {
  ...
  assert.match(ownerLegal, /min=\{viennaCalendarDate\(\)\}/);
});
