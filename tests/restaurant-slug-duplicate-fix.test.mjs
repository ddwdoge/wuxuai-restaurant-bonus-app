...
import {
  buildRestaurantActivationPatch,
  indexRowsByKey,
...
test("onboarding activation requires an existing restaurant and never inserts one", async () => {
  ...
  assert.match(source, /from\("restaurants"\)\s*\.update\(buildRestaurantActivationPatch\(input\)\)\s*\.eq\("id", restaurantId\)/);
});
...
test("activation updates status without changing an existing slug", () => {
  ...
  const patch = buildRestaurantActivationPatch({
  ...
  assert.equal(patch.onboarding_status, "completed");
  assert.equal(Object.hasOwn(patch, "slug"), false);
  assert.equal({ ...existingRestaurant, ...patch }.slug, "existing-slug");
});
