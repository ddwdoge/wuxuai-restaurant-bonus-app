...
export function RewardsPage() {
  ...
  return (
    <div className="premium-owner-management-page">
      ...
      <AppDrawer className="owner-mobile-drawer" fitVisualViewport description="Produkt, Preis, Foto und automatische Punkteberechnung." dismissOnOverlay={false} footer={editorOpen && !editingOffer ? wizardActions : null} onClose={closeRewardEditor} open={editorOpen && !editingOffer} size="large" title="Neue Punkteeinlösung">{wizardContent}</AppDrawer>

      <AppDrawer className="owner-mobile-drawer" fitVisualViewport description="Produkt, Preis, Foto und automatische Punkteberechnung bearbeiten." dismissOnOverlay={false} footer={editingOffer ? wizardActions : null} onClose={closeRewardEditor} open={editorOpen && Boolean(editingOffer)} size="large" title="Punkteeinlösung bearbeiten">{wizardContent}</AppDrawer>

      <AppDrawer className="owner-mobile-drawer" fitVisualViewport description="So sehen Gäste dieses Angebot im Kundenportal." footer={<button className="button" onClick={() => setPreviewOffer(null)} type="button">Vorschau schließen</button>} onClose={() => setPreviewOffer(null)} open={Boolean(previewOffer)} title="Vorschau im Kundenportal">
        {previewOffer ? <div className="premium-owner-preview-shell"><div className="customer-premium-shell customer-home-compact premium-owner-customer-reward-preview"><RewardCard category={previewOffer.category ?? "Eigenes Produkt"} imageCrop={rewardImageCropFromRecord(previewOffer)} imageFirst imageUrl={previewOffer.image_url} meta={`${previewOffer.required_points} Punkte`} state="available" status={previewOffer.active ? "Im Kundenportal sichtbar" : "Derzeit nicht sichtbar"} title={previewOffer.title} /></div><p className="premium-owner-preview-note"><Eye size={18} />Diese Vorschau löst keine Punkteeinlösung aus.</p></div> : null}
      </AppDrawer>

      <AppDrawer className="owner-mobile-drawer" fitVisualViewport description={pendingStatusOffer?.active ? "Neue Kunden können sie danach nicht mehr einlösen." : "Sie wird danach im Kundenportal sichtbar."} footer={pendingStatusOffer ? <><button className="button secondary" onClick={() => setPendingStatusOffer(null)} type="button">Abbrechen</button><button className="button" disabled={saving} onClick={() => toggleOffer(pendingStatusOffer)} type="button">{pendingStatusOffer.active ? "Deaktivieren" : "Aktivieren"}</button></> : null} onClose={() => setPendingStatusOffer(null)} open={Boolean(pendingStatusOffer)} size="compact" title={pendingStatusOffer?.active ? "Belohnung deaktivieren?" : "Belohnung aktivieren?"}>
      ...
      <AppDrawer className="owner-mobile-drawer" fitVisualViewport description="Nur Foto und Ausschnitt werden geändert. Punkte, Status und Bedingungen bleiben unverändert." dismissOnOverlay={false} footer={pendingQuickPhoto ? <><button className="button secondary" disabled={quickPhotoSaving} onClick={closeQuickPhoto} type="button">Abbrechen</button><button className="button" disabled={quickPhotoSaving || quickPhotoUnavailable} onClick={saveQuickPhoto} type="button">{quickPhotoSaving ? "Foto wird gespeichert …" : "Foto speichern"}</button></> : null} onClose={closeQuickPhoto} open={Boolean(pendingQuickPhoto)} size="compact" title="Bildausschnitt bearbeiten">
