...
export function WelcomeGiftsPage() {
  ...
  return (
    <div className="premium-owner-management-page">
      ...
      <AppDrawer className="owner-mobile-drawer" fitVisualViewport description="So sehen Gäste das Geschenk im Kundenportal." footer={<button className="button" onClick={() => setPreviewGift(null)} type="button">Vorschau schließen</button>} onClose={() => setPreviewGift(null)} open={Boolean(previewGift)} title="Vorschau im Kundenportal">
      ...
      <AppDrawer className="owner-mobile-drawer" fitVisualViewport description="Nur Foto und Ausschnitt werden geändert. Wert, Status und Bedingungen bleiben unverändert." dismissOnOverlay={false} footer={pendingQuickPhoto ? <><button className="button secondary" disabled={quickPhotoSaving} onClick={closeQuickPhoto} type="button">Abbrechen</button><button className="button" disabled={quickPhotoSaving || quickPhotoUnavailable} onClick={saveQuickPhoto} type="button">{quickPhotoSaving ? "Foto wird gespeichert …" : "Foto speichern"}</button></> : null} onClose={closeQuickPhoto} open={Boolean(pendingQuickPhoto)} size="compact" title="Bildausschnitt bearbeiten">
